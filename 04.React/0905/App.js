...
import { addMember } from './features/counter/userSlice';
...
function App() {
  ...
  // state.user : userSlice객체가 가지고 잇는 모든 데이터 (count, member)-> store.js key값을 불러옴(const rootReducer = combineReducers({user:userSlice})
  const [newUser, setNewUser] = useState({});
